<template>
  <div class="index-container">
    <label class="upload-btn">开始上传或拍照
      <input type="file" value="" accept="image/*" @click="getPhoto"
        id="image-input">
    </label>
    <div class="photo-box">
      <div class="preview-box">
        <img :src="imgSrc" id="preview-img"
          :style="{transform:'scale('+ myImg.scale+ ') translate('+myImg.position.x+'px,'+myImg.position.y+'px)'}"
          @touchstart="getPosition($event)" @touchmove="getMovePosition($event)"
          @touchend="getLeavePosition($event)">
      </div>
      <img :src="imgArr[curIndex]" id="preview-bg"
        @touchstart="getInitPosition($event)"
        @touchmove="getMovePosition($event)"
        @touchend="getLeavePosition($event)">
    </div>
    <div class="photo-ul">
        ...
        reads.addEventListener('load', function (e) {
          ...
          that.previewImg.addEventListener('load', function () {
            Exif.getData(that.previewImg, function () { // 获取图像的数据
              ...
              var rotateBase64 = rotateCanvas.toDataURL('image/jpeg', 1)
            })
          })
        })
      })
